    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive() && !isStopRequested()) {
            ...
            Pose2d driveDirection = new Pose2d();
                    ...
                    if (gamepad1.dpad_up == true && (holdTimerC.time() > 0.3) && (threadState == threadState.NULL || threadState == threadState.INTAKE) && drive.getShooterPower() == 0) {
                        if (targetVisible) {
                            //drive.setShooterServoPosition(0.95);
                            Pose2d poseReset = new Pose2d(poseEstimate.getX(), poseEstimate.getY(), Math.toRadians(-90));
                            drive.setPoseEstimate(poseReset);
                            ...
                            drive.turn(Math.toRadians(180 - error));
                            while (drive.isBusy()) {}
                            drive.switchState(threadState.SHOOT_SPIN);
                            holdTimerC.reset();
                        }
